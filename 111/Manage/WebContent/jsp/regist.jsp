...
<script type="text/javascript">
	...
	function checkUsername(){
		...
		var flag=true;
			...
			//alert("xxx");
		 	$.ajax({
				url:"${pageContext.request.contextPath}/jsp/user/checkUsername",
				type:"post",
				data:{
					"username":username
				},
	...
	function checkPassword() {
		...
		var replayLoginPassword=$.trim($("#repassword").val());
		if(null ==loginPassword || "" == loginPassword){
			$("#span2").html("请输入登录密码");
			return false;
		}else if(!/^[0-9a-zA-Z]+$/.test(loginPassword)){
			$("#span2").html("密码只可使用数字和大小写字母");
			return false;
		}else if(!/^(([a-zA-Z]+[0-9]+)|([0-9]+[a-zA-Z]+))[a-zA-Z0-9]*/.test(loginPassword)){
			$("#span2").html("密码应同时包含英文或数字");
			return false;
		}else if(loginPassword.length < 6 || loginPassword.length > 16){
			$("#span2").html("密码长度应为6-16");
			return false;
		}else{
			$("#span2").html("");
			return true;
		}
		if(null != loginPassword && null != replayLoginPassword && replayLoginPassword == loginPassword){
			$("#span3").html("");
			return true;
		} else {
			$("#span3").html("请检查确认密码");
	...
	function checkRePassword() {
		var loginPassword=$.trim($("#password").val());
		var replayLoginPassword=$.trim($("#repassword").val());
		if(null ==replayLoginPassword || ""==replayLoginPassword){
			$("#span3").html("确认密码不能为空");
			return false;
		}else if(null != loginPassword && null != replayLoginPassword && replayLoginPassword == loginPassword){
			$("#span3").html("");
		} else {
			$("#span3").html("密码和确认密码不一致");
		}
		return true;
	}
	/*  检查Email */
	...
	function regist() {
		...
		if(checkUsername()&&checkPassword()&&checkRePassword()&checkEmail()&checkPhone()){
			//$("#password").val($.md5(password));
		//	$("#repassword").val($.md5(replayLoginPassword));
			
			$.ajax({
				url:'${pageContext.request.contextPath}/jsp/user/register',
				type:'post',
				data:{
					username:username,
					password:password,
				...
				success:function(jsonObject){
					if(jsonObject.errorMessage == "ok") {
						window.location.href = "${pageContext.request.contextPath}/jsp/msg.jsp";
					} else {
						alert("注册失败");
					}
				},
...
<body>
						...
						<tbody>
							<tr>
								...
								<td><input type="text" id="username" name="username"
									class="text" maxlength="20" onblur="checkUsername()" /> <!-- <input type="text" id="span1"> -->
							...
							<tr>
								...
								<td><input type="password" id="password" name="password"
									...
									onblur="checkPassword()" /> <!-- <span><s:fielderror fieldName="password"/></span> -->
							...
							<tr>
								...
								<td><input id="repassword" type="password"
									...
									autocomplete="off" onblur="checkRePassword()" /> <span
									id="span3"></span></td>
							</tr>
							<tr>
								<th><span class="requiredField">*</span>E-mail:</th>
								<td><input type="text" id="email" name="email" class="text"
									maxlength="200" onblur="checkEmail()"> <!-- <span><s:fielderror fieldName="email"/></span> -->
									<span id="span4"></span></td>
							</tr>
							<tr>
								<th>姓名:</th>
							...
							<tr>
								...
								<td><input type="text" id="phone" name="phone" class="text"
									onblur="checkPhone()" /> <span id="span5"></span></td>
							</tr>

							<tr>
								<th>地址:</th>
								<td><input type="text" name="addr" id="addr" class="text"
							...
							<tr>
								...
								<td>
									...
									<button id="register" onclick="regist()">同意以下协议并注册</button>
								</td>
							</tr>
							<tr>
								<th>&nbsp;</th>
	...
	<div class="container footer">
		<div class="span24">
			<div class="footerAd">
				<img src="${pageContext.request.contextPath}/images/footer.jpg"
